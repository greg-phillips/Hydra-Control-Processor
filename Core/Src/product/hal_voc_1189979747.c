...
void init_voc_1189979747(uint16_t arg)
{
}
...
* @retval : None
*/
imx_result_t sample_voc_1189979747(uint16_t arg, void *value )
{
    if( sensor_error( SENSOR_CO2_VOC ) == true )
        return IMX_SENSOR_ERROR;
    if( hs.valid_tvoc_level == true ) {
        memcpy( value, &hs.current_tvoc_level, IMX_SAMPLE_LENGTH );
        return IMX_SUCCESS;
    } else
        return IMX_NO_DATA;
}
