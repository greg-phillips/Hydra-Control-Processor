...
/**@file
*
*    hal_particulate_matter_index_1695740845.c
*
...
void init_particulate_matter_index_1695740845(uint16_t arg)
{
}
...
* @retval : None
*/
imx_result_t sample_particulate_matter_index_1695740845(uint16_t arg, void *value )
{
    if( sensor_error( SENSOR_PARTICULATE_MATTER ) == true )
        return IMX_SENSOR_ERROR;
    if( hs.valid_particulate_matter == true ) {
        memcpy( value, &hs.current_PM_index, IMX_SAMPLE_LENGTH );
        return IMX_SUCCESS;
    } else
        return IMX_NO_DATA;
}
