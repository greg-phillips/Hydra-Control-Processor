...
/**@file
*
*    hal_typical_particle_size_159299661.c
*
...
void init_typical_particle_size_159299661(uint16_t arg)
{
}
...
* @retval : None
*/
imx_result_t sample_typical_particle_size_159299661(uint16_t arg, void *value )
{
    if( sensor_error( SENSOR_PARTICULATE_MATTER ) == true )
        return IMX_SENSOR_ERROR;
    if( hs.valid_particulate_matter == true ) {
        memcpy( value, &hs.current_typical_particle, IMX_SAMPLE_LENGTH );
        return IMX_SUCCESS;
    } else
        return IMX_NO_DATA;}
