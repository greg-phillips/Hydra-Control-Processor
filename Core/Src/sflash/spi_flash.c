...
/** @file spi_flash.c
 *
 * ST SPI FLASH Driver from: https://github.com/JoeMerten/Stm32/tree/master/Lib/Stm/Stm32F2xx_StdPeriph_Lib_V1.1.0/Project/STM32F2xx_StdPeriph_Examples/SPI/SPI_FLASH
 *
 ...
 ******************************************************/
/**
  * @brief	Erase a sector (4K) in the SFLASH
  * @param  Sector Number
  * @retval : None
  */
void erase_sector( uint16_t sector )
{
	sFLASH_EraseSector( ( (uint32_t) sector ) * 4096 );
}
/**
  ******************************************************************************
...
uint8_t sFLASH_SendByte(uint8_t byte)
{
  ...
  /*!< Send byte through the SPI peripheral */
	result = HAL_SPI_TransmitReceive( sFLASH_SPI, &byte, &read_value, 1, 100 );
	if( result != HAL_OK )
		printf( "Failed to Send SPI Byte: %u\r\n", result );
...
uint16_t sFLASH_SendHalfWord(uint16_t HalfWord)
{
	...
	result = HAL_SPI_TransmitReceive( sFLASH_SPI, (uint8_t *) &HalfWord, &read_value, 2, 100 );
	if( result != HAL_OK )
		printf( "Failed to Send SPI Byte: %u\r\n", result );
